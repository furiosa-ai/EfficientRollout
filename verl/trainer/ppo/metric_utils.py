...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    metrics = {
        ...
        **(
        ...
        # DIAGNOSTIC: std + sum are needed to probe H_PACKING
        # (wall-clock timers scale with total token count per step, and
        # length-variance drives the bimodal FAST/SLOW regime if present).
        "response_length/std": torch.std(response_length).detach().item(),
        "response_length/sum_tokens": torch.sum(response_length).detach().item(),
...
def compute_timing_metrics(batch: DataProto, timing_raw: dict[str, float]) -> dict[str, Any]:
    ...
    num_tokens_of_section = {
        ...
        "requantize": num_response_tokens,
        **{name: num_overall_tokens for name in ["ref", "old_log_prob", "values", "adv", "update_critic", "update_actor"]},
    }
