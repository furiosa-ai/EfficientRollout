...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self):
        ...
        # R2 DIAGNOSTIC: force_sync=False to isolate H_CHURN.
        # The original force_sync=True calls device.synchronize() which
        # drains every CUDA stream (pipeline stall). 3× per step could
        # explain the SLOW regime even without allocator retries.
        ...
        # DIAGNOSTIC (remove after root-cause confirmed): log allocator
        # state right after expandable_segments toggle to False. Paired
        # with trainer_mode's post-True log so we can see per-mode drift.
        try:
            import os as _os
            if int(_os.environ.get("RANK", "0")) == 0:
                _s = torch.cuda.memory_stats()
                print(
                    f"[alloc-diag] rollout_mode toggle(False): "
                    f"cum_retries={_s.get('num_alloc_retries', 0)} "
                    f"ooms={_s.get('num_ooms', 0)} "
                    f"inactive_split={_s.get('inactive_split_bytes.all.current', 0) / 1e9:.2f}GB "
                    f"reserved={_s.get('reserved_bytes.all.current', 0) / 1e9:.2f}GB "
                    f"allocated={_s.get('allocated_bytes.all.current', 0) / 1e9:.2f}GB",
                    flush=True,
                )
        except Exception as _e:
            print(f"[alloc-diag] rollout_mode failed: {_e}", flush=True)
        ...
        # Pass FSDP activation stats/samples to vLLM only when AWQ is active.
        # RTN mode has no collector → skip to avoid unnecessary getattr overhead.
        extra_kwargs = {}
        if hasattr(self, 'actor') and getattr(self.actor, '_activation_stats_collector', None) is not None:
            extra_kwargs['activation_stats'] = self.actor.last_activation_stats
            extra_kwargs['activation_samples'] = self.actor.last_activation_samples
        ...
        # Capture requantize overhead for per-step metrics
        self._last_requantize_sec = getattr(self.rollout, '_last_requantize_sec', None) or 0.0
    ...
    async def trainer_mode(self):
        ...
        # DIAGNOSTIC (remove after FAST/SLOW root-cause confirmed, 2026-04-23):
        # Rank-0 allocator state at trainer-mode entry. Logs delta of
        # num_alloc_retries per step (=cudaMalloc fallback count — direct
        # indicator of H_CHURN/H_ALLOC), plus reserved/inactive_split to
        # show fragmentation trend. If retries_delta > 0 on SLOW steps and
        # == 0 on FAST, allocator-driven explanation is confirmed.
        # Uses print() because fsdp_workers.logger has no attached handler
        # — its warnings never reach the main log. print → Ray stdout
        # capture → main smoke log.
        try:
            import os as _os
            if int(_os.environ.get("RANK", "0")) == 0:
                _s = torch.cuda.memory_stats()
                _prev = getattr(self, "_prev_alloc_retries", 0)
                _cur = _s.get("num_alloc_retries", 0)
                _delta = _cur - _prev
                self._prev_alloc_retries = _cur
                print(
                    f"[alloc-diag] trainer_mode entry: "
                    f"retries_delta={_delta} (cum={_cur}) "
                    f"ooms={_s.get('num_ooms', 0)} "
                    f"inactive_split={_s.get('inactive_split_bytes.all.current', 0) / 1e9:.2f}GB "
                    f"reserved={_s.get('reserved_bytes.all.current', 0) / 1e9:.2f}GB "
                    f"allocated={_s.get('allocated_bytes.all.current', 0) / 1e9:.2f}GB",
            ...
            print(f"[alloc-diag] trainer_mode failed: {_e}", flush=True)
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # Set up FSDP activation stats collector only for AWQ mode.
            # RTN doesn't need activation stats — skip to avoid hook overhead.
            if getattr(self.config.rollout, 'spec_method', None) == 'quant_self' \
                    and getattr(self.config.rollout, 'spec_quant_method', 'rtn') == 'awq':
                calibration_prompt_num = getattr(
                    self.config.rollout, 'spec_awq_calibration_prompts', 1
                )
                self.actor.setup_activation_stats_collector(
                    calibration_prompt_num=int(calibration_prompt_num),
                )
    ...
    def update_actor(self, data: DataProto):
        ...
        # DIAGNOSTIC (remove after root-cause confirmed, 2026-04-23):
        # sub-phase timing for update_actor investigation — separate
        # load/compute/offload to see where wall-clock 41-47s spends.
        import os as _os
        import time as _time
        _diag_rank0 = int(_os.environ.get("RANK", "0")) == 0
        _t0 = _time.time() if _diag_rank0 else 0.0

        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=get_device_id())
        _t_load = _time.time() - _t0 if _diag_rank0 else 0.0

        _t1 = _time.time() if _diag_rank0 else 0.0
        ...
        _t_compute = _time.time() - _t1 if _diag_rank0 else 0.0

        _t2 = _time.time() if _diag_rank0 else 0.0
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
        ...
        _t_offload = _time.time() - _t2 if _diag_rank0 else 0.0

        if _diag_rank0:
            _total = _t_load + _t_compute + _t_offload
            print(
                f"[phase-diag] update_actor: "
                f"load={_t_load:.2f}s compute={_t_compute:.2f}s "
                f"offload={_t_offload:.2f}s (subtotal={_total:.2f}s)",
                flush=True,
            )
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # Inject requantize overhead so it flows to ray_trainer timing_raw
        requantize_sec = getattr(self, '_last_requantize_sec', 0.0)
        if requantize_sec > 0:
            timing_generate["requantize"] = requantize_sec
        self._last_requantize_sec = 0.0  # Reset to prevent stale propagation
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        assert self._is_actor
        # DIAGNOSTIC (remove after root-cause confirmed, 2026-04-23):
        # sub-phase timing for old_log_prob investigation. ray_trainer's
        # marked_timer sees wall-clock 13-15s on our setup vs 8.3s on
        # "FAST" baseline runs. Decomposes that into load/compute/reshard/
        # offload to isolate which phase drives the +5s overhead.
        import os as _os
        import time as _time
        _diag_rank0 = int(_os.environ.get("RANK", "0")) == 0
        _t0 = _time.time() if _diag_rank0 else 0.0
        ...
        _t_reshard = _time.time() - _t2 if _diag_rank0 else 0.0

        _t3 = _time.time() if _diag_rank0 else 0.0
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_log_prob", logger=logger)
        _t_offload = _time.time() - _t3 if _diag_rank0 else 0.0

        if _diag_rank0:
            _total = _t_load + _t_compute + _t_reshard + _t_offload
            print(
                f"[phase-diag] compute_log_prob (is_lora={is_lora}): "
                f"load={_t_load:.2f}s compute={_t_compute:.2f}s "
                f"reshard={_t_reshard:.2f}s offload={_t_offload:.2f}s "
                f"(subtotal={_total:.2f}s)",
                flush=True,
            )
