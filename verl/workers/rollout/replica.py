...
class TokenOutput(BaseModel):
    ...
    """number of out-of-top-N logprob fallback events (SD recovery tokens
    sampled outside vLLM's top-N logprobs dict). 0 in normal sampling."""
    first_entry_mismatch_count: int = 0
    """positions where dict's first entry token_id != token_ids[i]. Should
    be 0 if the SD invariant (first entry = final accepted token's target
    logprob) holds. Nonzero indicates a code path where the invariant
    breaks — value still used (it's the rollout policy's logprob) but
    surfaces a hypothesis violation worth investigating."""
...
class RolloutReplica(ABC):
    ...
    # TODO: this should be the default solution, but need to make the RolloutMode more clear.
    ...
    async def wake_up(self):
        """Wake up each rollout server. Returns max requantize_sec across servers."""
        results = await asyncio.gather(*[server.wake_up.remote() for server in self.servers])
        return max((r if isinstance(r, (int, float)) and not isinstance(r, bool) else 0.0) for r in results)
    ...
    async def set_validate_mode(self, mode: bool):
        """Propagate adaptive-γ validation-mode flag to all servers.

        Uses ``return_exceptions=True`` so a single server failure
        does not leave other in-flight RPCs silently completing
        behind the first raise (default ``asyncio.gather`` semantics):
        such a partial apply could leave some engines stuck with
        ``_validate_mode=True`` and contaminate the next training
        rollout's adaptive-γ state. Any exception → raise the first
        one seen so the caller's try/finally observes the failure.
        """
        results = await asyncio.gather(
            *[server.set_validate_mode.remote(mode) for server in self.servers],
