...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_worker(self, all_kwargs: list[dict[str, Any]]):
        ...
        # --- Quantized Self-Speculative Decoding (Phase 2e) ---
        if getattr(self.config, 'spec_method', None) == 'quant_self':
            from vllm.config.speculative import SpeculativeConfig
            spec_dict = {
                "method": "quant_self",
                "num_speculative_tokens": getattr(
                    self.config, 'spec_num_draft_tokens', 3
                ),
                "quant_method": getattr(
                    self.config, 'spec_quant_method', 'rtn'
                ),
                "quant_group_size": getattr(
                    self.config, 'spec_quant_group_size', 128
                ),
            ...
            if getattr(self.config, 'spec_sd_toggle_threshold', None) is not None:
                spec_dict["sd_toggle_threshold"] = self.config.spec_sd_toggle_threshold
            if getattr(self.config, 'spec_sd_toggle_mode', "off") != "off":
                spec_dict["sd_toggle_mode"] = self.config.spec_sd_toggle_mode
            if getattr(self.config, 'spec_sd_toggle_config', None) is not None:
                spec_dict["sd_toggle_config_path"] = self.config.spec_sd_toggle_config
            spec_dict["sd_toggle_margin"] = float(
            ...
            if _ladder_str:
                ...
                spec_dict["gamma_ladder"] = _ladder
                # Adaptive-γ: start at ladder[0], scheduler sizes to max(ladder)
                spec_dict["num_speculative_tokens"] = _ladder[0]
            self.vllm_config.speculative_config = SpeculativeConfig(**spec_dict)
            logger.info(
                "Quantized self-SD enabled: gamma=%d, quant=%s, toggle=%s",
                self.config.spec_num_draft_tokens,
                spec_dict["quant_method"],
                spec_dict.get("sd_toggle_threshold", "always-on"),
            )
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        if peft_config and base_sync_done:
            ...
            # Post-weight-sync: requantize drafter if quantized SD is enabled
            if hasattr(model_runner, 'on_weights_updated'):
                activation_stats = kwargs.get('activation_stats', None)
                activation_samples = kwargs.get('activation_samples', None)
                requantize_sec = model_runner.on_weights_updated(
                    activation_stats=activation_stats,
                    activation_samples=activation_samples,
                )
                self._last_requantize_sec = requantize_sec or 0.0
