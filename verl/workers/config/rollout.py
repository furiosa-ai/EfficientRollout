...
class RolloutConfig(BaseConfig):
    ...
    frequency_penalty: float = 0.0  # vLLM SamplingParams; 0.0 = disabled, valid range [-2.0, 2.0]
    presence_penalty: float = 0.0   # vLLM SamplingParams; 0.0 = disabled, valid range [-2.0, 2.0]
    ...
    allocation_policy: str = "default"
    tail_amplification: float = 2.0
    ema_alpha: float = 0.3

    # --- Quantized Self-Speculative Decoding ---
    spec_method: Optional[str] = None          # "quant_self" | None
    spec_num_draft_tokens: int = 3             # gamma (γ)
    spec_quant_method: str = "rtn"             # "rtn" | "awq" (quantized self-drafter)
    spec_quant_group_size: int = 128           # W4 group size
    spec_awq_calibration_prompts: int = 1      # prompts to collect for AWQ calibration
    spec_sd_toggle_mode: str = "off"                     # "off" | "threshold" | "roofline"
    spec_sd_toggle_threshold: Optional[int] = None       # batch count threshold / ramp-up guard
    spec_sd_toggle_config: Optional[str] = None          # path to sd_toggle config JSON (mode="roofline")
    spec_sd_toggle_margin: float = 0.05                  # roofline: require sp_pred >= 1 + margin
    spec_gamma_ladder: Optional[str] = None              # adaptive-γ ladder, e.g. "5,7,9,11" (None = fixed-γ)
    ...
    def __post_init__(self):
        ...
        if self.spec_method is not None and self.spec_method not in {"quant_self"}:
            raise ValueError(
                f"spec_method must be 'quant_self' or None, got '{self.spec_method}'"
            )
        if self.spec_quant_method not in {"rtn", "awq"}:
            raise ValueError(
                f"spec_quant_method must be 'rtn' or 'awq', got '{self.spec_quant_method}'"
            )
        if self.spec_num_draft_tokens < 1:
            raise ValueError(
                f"spec_num_draft_tokens must be >= 1, got {self.spec_num_draft_tokens}"
            )
        if self.spec_sd_toggle_margin < 0.0:
            raise ValueError(
                f"spec_sd_toggle_margin must be >= 0.0, got {self.spec_sd_toggle_margin}"
            )
        if self.spec_sd_toggle_margin > 1.0:
            raise ValueError(
                "spec_sd_toggle_margin > 1.0 effectively disables SD entirely; "
                f"got {self.spec_sd_toggle_margin}. Use spec_sd_toggle_mode='off' instead."
            )
        if self.spec_gamma_ladder is not None:
            # Parse and validate comma-separated ladder
            try:
                _ladder = [int(x.strip()) for x in self.spec_gamma_ladder.split(",") if x.strip()]
            except ValueError:
                raise ValueError(
                    f"spec_gamma_ladder must be comma-separated ints (e.g., '3,5,7,9,11'), "
                    f"got {self.spec_gamma_ladder!r}"
                )
            if len(_ladder) < 2:
                raise ValueError(
                    f"spec_gamma_ladder must have >= 2 values for elevation, got {_ladder}"
                )
            if _ladder != sorted(_ladder) or len(set(_ladder)) != len(_ladder):
                raise ValueError(
                    f"spec_gamma_ladder must be strictly ascending, got {_ladder}"
                )
            if any(g < 1 for g in _ladder):
                raise ValueError(f"spec_gamma_ladder values must be >= 1, got {_ladder}")
            if self.spec_method != "quant_self":
                raise ValueError(
                    f"spec_gamma_ladder requires spec_method='quant_self', "
                    f"got '{self.spec_method}'"
                )
            _deltas = [_ladder[i + 1] - _ladder[i] for i in range(len(_ladder) - 1)]
            if any(d != 2 for d in _deltas):
                raise ValueError(
                    f"spec_gamma_ladder must use uniform step Δγ=2 "
                    f"(e.g., '5,7,9,11'), got {_ladder} (deltas={_deltas})"
                )
