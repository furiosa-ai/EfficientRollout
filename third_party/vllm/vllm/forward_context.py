...
class BatchDescriptor(NamedTuple):
    ...
    """
    Per-request query length for uniform decode batches.
    AR decode = 1, SD verify = 1 + num_spec_tokens.
    Used to distinguish AR FULL and SD FULL graphs at the same num_tokens.
    """
    ...
    def non_uniform(self) -> "BatchDescriptor":
        ...
        return BatchDescriptor(
            ...
            decode_query_len=1,  # PIECEWISE keys always use qlen=1
        )
...
class ForwardContext:
    ...
    # Override sliding window for self-SD draft passes; None = use layer default
    draft_window_size: tuple[int, int] | None = None

    # --- Phase 1b: Verification-guided sparse attention ---
    # Query collection: save query tensors during verification for KV selection.
    collect_queries: bool = False
    collect_query_layers: frozenset[str] | None = None
    collected_queries: dict[str, "torch.Tensor"] | None = None
    # Optional query indices to collect from current forward.
    # If set, collect only query[collect_query_indices].
    collect_query_indices: torch.Tensor | None = None
    # Optional vector buffer for query collection (layer_name -> tensor).
    collected_query_vectors: dict[str, torch.Tensor] | None = None
    # Sparse KV override: during draft passes, replace FlashAttention's KV read
    # with a gathered virtual KV cache. Writes still go to real KV cache.
    # Outer dict is keyed by layer_name; inner dict keys are:
    # "block_table", "seq_lens", "max_seqlen_k", "sparse_capacity_tokens"
    sparse_kv_override: dict[str, dict[str, object]] | None = None
...
def create_forward_context(
    ...
    collect_queries: bool = False,
    collect_query_layers: frozenset[str] | None = None,
    collected_queries: dict | None = None,
    collect_query_indices: torch.Tensor | None = None,
    collected_query_vectors: dict | None = None,
    sparse_kv_override: dict | None = None,
):
    return ForwardContext(
        ...
        collect_query_layers=collect_query_layers,
        collected_queries=collected_queries,
        collect_query_indices=collect_query_indices,
        collected_query_vectors=collected_query_vectors,
        sparse_kv_override=sparse_kv_override,
    )
