...
class SamplerOutput:
    ...
    # Per-request count of logprob rows in `logprobs_tensors`. Single
    # source-of-truth for cu_num_accepted_tokens construction in the
    # downstream worker bookkeeping. Populated only by RejectionSampler
    # (SD path). For non-SD `Sampler.forward`, leaves as None — callers
    # fall back to deriving cu_num from valid_sampled_token_ids lengths.
    # shape [num_reqs], int on the same device as logprobs_tensors.
