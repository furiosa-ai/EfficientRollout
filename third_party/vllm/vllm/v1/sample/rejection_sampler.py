...
class RejectionSampler(nn.Module):
    ...
    def forward(
        ...
        # Debug: log draft_probs status for debugging acceptance rate.
        if draft_probs is None:
            logger.debug(
                "rejection_sampler: draft_probs is None, "
                "using NO_DRAFT_PROBS mode")
        ...
        # Bug: the previous truthy check `if max_num_logprobs:` silently dropped
        # the `logprobs=0` (sampled-token-only) case, since 0 is falsy. Under
        # speculative decoding this set logprobs_tensors=None on every SD step →
        # ModelRunnerOutput.logprobs=None → scheduler's `and logprobs` False →
        # EngineCoreOutput.new_logprobs=None → CompletionOutput.logprobs
        # accumulator fell behind token_ids by γ per SD step (raw_logprobs
        # became a strict prefix of token_ids). 264 mismatch hits over 5 steps
        # in a Qwen-2.5-14B SD canary, all with raw_logprobs[:5] keys matching
        # token_ids[:5] but token_ids extending past raw_logprobs end.
        #
        # Fix: switch to `is not None`, matching Sampler.forward (sampler.py:80).
        # This is a 2-way switch (None vs >=0). The -1 sentinel that Sampler
        # supports (sampler.py:104-108, "return entire raw_logprobs") is
        # STRUCTURALLY UNREACHABLE here: gpu_input_batch.py:382-386 rewrites
        # external `logprobs=-1` to `vocab_size` before SamplingMetadata is
        # built, and the only -1 in this file is the internal bonus_sampler
        # call at line 105 which routes to Sampler.forward, not back into
        # RejectionSampler.forward.
        logprobs_tensors = None
        accepted_row_counts: torch.Tensor | None = None
        if sampling_metadata.max_num_logprobs is not None:
            logprobs_tensors, accepted_row_counts = self._get_logprobs_tensors(
            ...
            # Self-consistency check on _get_logprobs_tensors itself: the
            # gather_logprobs row count must equal accepted_mask.sum() because
            # _get_logprobs_tensors uses the same `!= PLACEHOLDER_TOKEN_ID`
            # predicate (line 211) to build accepted_mask. This guards against
            # a future refactor that decouples the two — it does NOT catch the
            # parse_output / _get_logprobs_tensors vocab_size asymmetry
            # (parse_output additionally requires `< vocab_size`); that
            # asymmetry would surface at the downstream sync-path assertion in
            # gpu_model_runner.py (cu_num is built from valid_sampled_token_ids
            # which uses parse_output's stricter predicate).
            _expected_rows = int(
                (output_token_ids != PLACEHOLDER_TOKEN_ID).sum().item()
            )
            assert logprobs_tensors.logprobs.shape[0] == _expected_rows, (
                f"rejection_sampler logprobs_tensors row count "
                f"{int(logprobs_tensors.logprobs.shape[0])} != accepted_mask sum "
                f"{_expected_rows}"
            )
            assert int(accepted_row_counts.sum().item()) == _expected_rows, (
                f"rejection_sampler accepted_row_counts sum "
                f"{int(accepted_row_counts.sum().item())} != accepted_mask sum "
                f"{_expected_rows}"
            )
        ...
        return SamplerOutput(
            ...
            accepted_row_counts=accepted_row_counts,
        )
    ...
    def _get_logprobs_tensors(
    ...
    ) -> tuple[LogprobsTensors, torch.Tensor]:
        ...
        # Return num_accepted_tokens as the per-request row-count SoT for
        # downstream cu_num construction. Computed here from the same
        # accepted_mask used to build logprobs rows, so the two are aligned
        # by construction.
        return logprobs_tensors, num_accepted_tokens
