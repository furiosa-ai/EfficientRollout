...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if speculative_config:
            # Adaptive-γ: when a ladder is configured, γ can elevate up to
            # max(ladder) during training. Scheduler's num_spec_tokens is
            # used to size SpecDecodingStats (assert num_accepted <=
            # num_spec_tokens) and KV lookahead allocation. Both must be
            # sized for the MAX γ we'll ever see, otherwise post-elevation
            # asserts fire when a higher-γ accepts more than initial γ.
            _ladder = getattr(speculative_config, "gamma_ladder", None)
            if _ladder:
                self.num_spec_tokens = max(_ladder)
            else:
                self.num_spec_tokens = speculative_config.num_speculative_tokens
            if speculative_config.use_eagle():
                self.use_eagle = True
                self.num_lookahead_tokens = self.num_spec_tokens
            elif speculative_config.method == "sparse_attn":
                # Self-SD also writes draft tokens to KV cache; need lookahead slots.
                self.num_lookahead_tokens = self.num_spec_tokens
        # For sparse_attn WITH a toggle threshold, SD starts disabled until
        # SD starts disabled when any toggle mode is active (threshold or roofline).
        # The monotone toggle in EngineCore enables it when conditions are met.
        _has_toggle = (
            speculative_config
            and getattr(speculative_config, "sd_toggle_mode", "off") != "off"
        )
        # Backward compat: old configs without sd_toggle_mode but with threshold
        if not _has_toggle and speculative_config and getattr(
            speculative_config, "sd_toggle_threshold", None
        ) is not None:
            _has_toggle = True
        self._spec_decode_enabled = not _has_toggle
    ...
    def enable_spec_decode(self) -> None:
        """Monotone toggle: enable speculative decoding for all running requests.

        Cold start: the first schedule() after this produces 0 spec_decode_tokens
        because no draft tokens exist yet. The proposer runs after the first
        post-toggle execute_model(), so the SECOND schedule() call will have
        draft tokens. This one-tick delay is expected behavior, NOT a bug.
        """
        self._spec_decode_enabled = True

    def disable_spec_decode(self) -> None:
        """Reset SD toggle for a new RL rollout step.

        Called from EngineCore.wake_up() to ensure each RL step starts
        with SD disabled and re-evaluates the toggle condition.
        """
    ...
    def update_draft_token_ids(
        ...
        # For sparse_attn self-SD: ignore draft tokens until toggle fires.
    ...
    def get_decode_state(self) -> tuple[int, float]:
        """Return (n_active, avg_seq_len) for SD toggle decisions.

        n_active: number of currently running (decoding) requests.
        avg_seq_len: mean total sequence length (prompt + generated tokens)
                     of active requests.

        Cost: O(n_active), negligible compared to the GPU forward pass.
        """
        n = len(self.running)
        if n == 0:
            return (0, 0.0)
        total_len = sum(req.num_tokens for req in self.running)
        return (n, total_len / n)
    ...
    def make_spec_decoding_stats(
        ...
        # Always track SD stats when SD is active, regardless of log_stats.
        # veRL uses --disable_log_stats but still needs acceptance metrics
        # for the _SDStatsAccumulator in AsyncLLM.
        if not self.log_stats and self.num_spec_tokens == 0:
            return None
