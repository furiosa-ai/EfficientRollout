...
class SchedulerStats:
    ...
    NOTE: All fields MUST have defaults for IPC compatibility.
    This dataclass is nested inside EngineCoreOutputs (msgspec.Struct with
    omit_defaults=True). Adding a field without a default will break
    ...
    # SD toggle state from EngineCore (for veRL profiler bridge)
    sd_toggled: bool = False
    sd_toggle_tick: int = -1
    sd_toggle_batch: int = -1
    # L_accept used in roofline toggle this rollout (for observability)
    toggle_L_accept_used: float = 0.0
    # Adaptive-γ: current γ value used this rollout (fixed-γ reports constant value)
    sd_current_gamma: int = -1
    # Adaptive-γ: transition target γ (= γ_ladder[idx+1] when has next rung, else -1).
    # Pair (sd_current_gamma, sd_elevation_to_gamma) shows the next ladder rung
    # the current γ would elevate to.
    sd_elevation_to_gamma: int = -1
    # Adaptive-γ (AR-threshold): observed acceptance rate this rollout
    # (= total_accepted / total_drafted = (MAL - 1) / γ). Elevation fires when
    # sd_ar_observed >= AR_THRESHOLD (default 0.94), no warmup needed.
    # Sentinel -1.0 means "no SD rollout yet"; valid observations are in [0, 1].
    sd_ar_observed: float = -1.0
    sd_elevated_at_step: int = -1            # step at which γ last elevated (-1 if never)
