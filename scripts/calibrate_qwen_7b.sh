#!/bin/bash
# Calibrate SD toggle roofline model for Qwen2.5-7B
#
# Usage:
#   bash scripts/calibrate_qwen_7b.sh                         # A100 default, uses 5 GPUs (AR + 4 γ)
#   bash scripts/calibrate_qwen_7b.sh --gpus 0,1,2,3,4        # custom GPU subset (match pass count)
#
# Pipeline: multi-GPU sweep → fit → save config
# Output:  sd_toggle/configs/${gpu_short}_tp1_qwen257b.json (overwrite)
#          e.g. a100_tp1_qwen257b.json
#
...
SWEEP_CSV="results/recalibrate_${TIMESTAMP}/qwen_7b_sweep.csv"
...
FINAL_CSV="${CONFIG_DIR}/sweep_${GPU_SHORT}_tp${TP}_qwen257b.csv"
...
CONFIG_FILE="${CONFIG_DIR}/${GPU_SHORT}_tp${TP}_qwen257b.json"
