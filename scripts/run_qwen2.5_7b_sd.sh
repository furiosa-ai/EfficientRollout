#!/bin/bash
# veRL E2E: Qwen2.5-7B + GRPO with Quantized Self-Speculative Decoding
#
# Usage:
#   bash scripts/run_qwen2.5_7b_sd.sh no-sd      # No SD baseline
#   bash scripts/run_qwen2.5_7b_sd.sh rtn        # SD with RTN quantized drafter (always-on, fixed γ)
#   bash scripts/run_qwen2.5_7b_sd.sh toggle     # SD with roofline toggle (fixed γ)
#   bash scripts/run_qwen2.5_7b_sd.sh adaptive   # roofline toggle + adaptive-γ (full EfficientRollout)
#
...
SPEC_NUM_DRAFT_TOKENS=${SPEC_NUM_DRAFT_TOKENS:-7}
GPU_SHORT="a100"
SD_TOGGLE_CONFIG=${SD_TOGGLE_CONFIG:-"${PROJECT_ROOT}/sd_toggle/configs/${GPU_SHORT}_tp1_qwen257b.json"}

SD_ARGS=""
case $MODE in
  no-sd)
    echo "=== Qwen2.5-7B No-SD Baseline ==="
    SD_ARGS=""
    EXPERIMENT_NAME="qwen7b_baseline_nosd"
    ;;
  rtn)
    echo "=== Qwen2.5-7B SD with RTN Quantized Drafter (always-on) ==="
    ...
    EXPERIMENT_NAME="qwen7b_sd_rtn"
    ;;
  toggle)
    echo "=== Qwen2.5-7B SD with roofline toggle (margin=${SD_TOGGLE_MARGIN}) ==="
    ...
    EXPERIMENT_NAME="qwen7b_sd_toggle_roofline"
    ;;
  adaptive)
    echo "=== Qwen2.5-7B EfficientRollout: roofline toggle + adaptive-γ (ladder=${SPEC_GAMMA_LADDER}) ==="
    ...
    EXPERIMENT_NAME="qwen7b_sd_adaptive"
    ;;
  *)
