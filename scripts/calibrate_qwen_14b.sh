#!/bin/bash
# Calibrate SD toggle roofline model for Qwen2.5-14B (TP=1)
#
# Usage:
#   bash scripts/calibrate_qwen_14b.sh                              # A100 default, uses 5 GPUs (AR + 4 γ)
#   bash scripts/calibrate_qwen_14b.sh --gpus 0,1,2,3,4             # custom GPU subset (match pass count)
#
# GAMMAS=3,7,11,15 → 5 passes (AR/γ=0, γ=3, γ=7, γ=11, γ=15). 5 GPUs match 1:1.
#
# Pipeline: multi-GPU sweep → fit → save config
# Output:  sd_toggle/configs/${gpu_short}_tp1_qwen2514b.json (overwrite)
#          e.g. a100_tp1_qwen2514b.json
#
# Time estimate: ~25-30 min (5 GPUs, 1 pass/GPU)
#
# This script intentionally fits the TP=1 config consumed by the current
# Qwen2.5-14B launcher (`scripts/run_qwen2.5_14b_sd.sh`).
...
# Default: 5 GPUs parallel (5 sweep passes × 1 GPU each, AR + γ∈{3,7,11,15})
GPUS_ARG="0,1,2,3,4"
while [[ $# -gt 0 ]]; do
    case $1 in
...
SWEEP_CSV="results/recalibrate_${TIMESTAMP}/qwen_14b_sweep.csv"
...
echo "============================================================"
echo "SD Toggle Calibration: ${MODEL_SHORT} (TP=${TP})"
echo "============================================================"
echo "  GPUs:     ${GPUS_ARG}"
echo "  TP:       ${TP}"
echo "  Gammas:   ${GAMMAS}"
...
FINAL_CSV="${CONFIG_DIR}/sweep_${GPU_SHORT}_tp${TP}_qwen2514b.csv"
...
CONFIG_FILE="${CONFIG_DIR}/${GPU_SHORT}_tp${TP}_qwen2514b.json"
