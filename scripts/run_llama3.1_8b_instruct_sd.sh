#!/bin/bash
# veRL E2E: LLaMA-3.1-8B-Instruct + GRPO with Quantized Self-Speculative Decoding
#
# Usage:
#   bash scripts/run_llama3.1_8b_instruct_sd.sh no-sd      # No SD baseline
#   bash scripts/run_llama3.1_8b_instruct_sd.sh rtn        # SD with RTN quantized drafter (always-on, fixed γ)
#   bash scripts/run_llama3.1_8b_instruct_sd.sh toggle     # SD with roofline toggle (fixed γ)
#   bash scripts/run_llama3.1_8b_instruct_sd.sh adaptive   # roofline toggle + adaptive-γ (full EfficientRollout)
#
# Hardware: 8x A100-SXM4-80GB (1 node)
# Data: simplerl-8k-medium-llama-instruct (MATH lv.1-4)
...
DATA_DIR=${DATA_DIR:-"${PROJECT_ROOT}/data/simplerl-8k-medium-llama-instruct"}
...
GPU_SHORT="a100"
SD_TOGGLE_CONFIG=${SD_TOGGLE_CONFIG:-"${PROJECT_ROOT}/sd_toggle/configs/${GPU_SHORT}_tp1_llama318binstruct.json"}

SD_ARGS=""
case $MODE in
  no-sd)
    echo "=== LLaMA-3.1-8B-Instruct No-SD Baseline ==="
    SD_ARGS=""
    EXPERIMENT_NAME="llama8b_instruct_baseline_nosd"
    ;;
  rtn)
    echo "=== LLaMA-3.1-8B-Instruct SD with RTN Quantized Drafter (always-on) ==="
    ...
    EXPERIMENT_NAME="llama8b_instruct_sd_rtn"
    ;;
  toggle)
    echo "=== LLaMA-3.1-8B-Instruct SD with roofline toggle (margin=${SD_TOGGLE_MARGIN}) ==="
    ...
    EXPERIMENT_NAME="llama8b_instruct_sd_toggle_roofline"
    ;;
  adaptive)
    echo "=== LLaMA-3.1-8B-Instruct EfficientRollout: roofline toggle + adaptive-γ (ladder=${SPEC_GAMMA_LADDER}) ==="
    ...
    EXPERIMENT_NAME="llama8b_instruct_sd_adaptive"
    ;;
  *)
